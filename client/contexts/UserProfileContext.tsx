...
export function UserProfileProvider({ children }: { children: ReactNode }) {
  ...
  const updatePriorities = async (priorityUpdates: Partial<PriorityWeights>) => {
    if (!profile) return;
  ...
  const completeOnboarding = async () => {
    if (!profile) return;
    const updated = await updateUserProfile({ hasCompletedOnboarding: true, onboardingStep: 3 });
    setProfile(updated);
  };
  ...
  const updatePrivacySettings = async (settingsUpdates: Partial<PrivacySettings>) => {
    if (!profile) return;
    const newSettings = { ...(profile.privacySettings || DEFAULT_PRIVACY_SETTINGS), ...settingsUpdates };
  ...
  const updateAppSettings = async (settingsUpdates: Partial<AppSettings>) => {
    if (!profile) return;
    const newSettings = { ...(profile.appSettings || DEFAULT_APP_SETTINGS), ...settingsUpdates };
