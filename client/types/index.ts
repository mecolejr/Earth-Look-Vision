...
export const CLIMATE_OPTIONS = {
    temperature: [
      { value: "cold", label: "Cold (Below 50 degrees F average)", description: "Love snow and bundling up" },
      { value: "mild", label: "Mild (50-65 degrees F average)", description: "Comfortable year-round temps" },
      { value: "warm", label: "Warm (65-80 degrees F average)", description: "Enjoy outdoor activities" },
      { value: "hot", label: "Hot (Above 80 degrees F average)", description: "Love the heat" },
        ],
    seasons: [
...
export interface CityScore {
    ...
    /**
     * Feature 1: When identity-specific hate crime data is available and
     * affects the safety score, this field explains why the personalised safety
     * score differs from the aggregate. Shown in the ScoreBreakdown UI.
     */
  safetyPersonalisationNote?: string;
}
 ...
 *
 * `hateCrimesByBiasCategory` breaks out FBI hate crime data by bias motivation.
 * Keys match the FBI's reporting categories and are mapped to user identity
 * fields in scoring.ts.
 *
 * All fields are optional because:
 * 1. Not all cities report to the FBI's NIBRS/UCR hate crime program.
 * 2. Some bias categories have too few incidents to publish without risk of
 *    identifying victims (FBI suppression rules).
 *
 * Values are incidents per 100,000 population (rate-normalised) so cities
 * of different sizes can be compared fairly.
 */
export interface HateCrimesByBiasCategory {
    /** Anti-Black or African American incidents per 100k */
  antiBlack?: number;
    /** Anti-White incidents per 100k */
  antiWhite?: number;
    /** Anti-Hispanic or Latino incidents per 100k */
  antiHispanic?: number;
    /** Anti-Asian incidents per 100k */
  antiAsian?: number;
    /** Anti-Arab incidents per 100k */
  antiArab?: number;
    /** Anti-Native American or Alaska Native incidents per 100k */
  antiNativeAmerican?: number;
    /** Anti-Jewish incidents per 100k */
  antiJewish?: number;
    /** Anti-Muslim incidents per 100k */
  antiMuslim?: number;
    /** Anti-other religion incidents per 100k */
  antiOtherReligion?: number;
    /** Anti-gay (male) incidents per 100k */
  antiGay?: number;
    /** Anti-lesbian incidents per 100k */
  antiLesbian?: number;
    /** Anti-bisexual incidents per 100k */
  antiBisexual?: number;
    /** Anti-transgender incidents per 100k */
  antiTransgender?: number;
    /** Anti-gender nonconforming incidents per 100k */
  antiGenderNonconforming?: number;
    /** Anti-disability incidents per 100k */
  antiDisability?: number;
}

export interface CitySafetyData {
    overallCrimeRate: number;
    /**
     * Legacy: aggregate hate crime counts by broad category.
  ...
  hateCrimesByCategory: Record<string, number>;
    ...
    /**
     * Feature 1: Granular FBI hate crime data by bias motivation category.
     * Optional — not all cities have this data. When present, scoring.ts uses
     * it to personalise the safety score based on the user's identity.
     */
  hateCrimesByBiasCategory?: HateCrimesByBiasCategory;
    /**
     * Feature 1: Year the hate crime data was last updated (e.g. 2023).
     * Used to show a "data vintage" notice in the UI.
     */
  hateCrimeDataYear?: number;
}
 ...
 *
 * Client-side type mirroring the DB testimonials table.
 * `id` and `createdAt` are populated by the server after insert.
 */
export interface Testimonial {
    ...
    content: string;
    rating: number;
    /** Identity dimensions the reviewer chose to share (all optional) */
  ethnicity?: string;
    genderIdentity?: string;
    sexualOrientation?: string;
    religion?: string;
...
/**
 * Feature 2 — Aggregate "people like you" rating for a city.
 * Returned by the server alongside individual testimonials.
 */
export interface TestimonialAggregate {
    cityId: string;
    averageRating: number;
    totalCount: number;
    /** Rating from testimonials matching the user's identity profile */
  identityMatchRating?: number;
    identityMatchCount?: number;
    /** Which identity dimension(s) were matched */
 ...
 *
 * A scenario is an ephemeral overlay on the user's real profile.
 * It overrides specific identity and/or priority fields without
 * persisting to storage. Passed directly to calculateCityScore()
 * with skipCache = true.
 */
export interface ScenarioProfile {
    /** Human-readable label shown in the UI */
  label: string;
    /** Description of what this scenario models */
  description: string;
    /** Partial identity overrides — only fields that differ from the real profile */
  identityOverrides?: Partial<IdentityProfile>;
    /** Partial priority overrides */
  priorityOverrides?: Partial<PriorityWeights>;
}

/** Pre-built scenario templates shown as quick-start options */
export const SCENARIO_TEMPLATES: ScenarioProfile[] = [
  {
        label: "If I have kids",
        description: "Re-weights school quality, safety, and family-friendliness",
        identityOverrides: { familyStructure: "Planning to have children" },
        priorityOverrides: { safety: 80, costOfLiving: 70 },
  },
  {
        label: "If I earn less",
        description: "Models how cost of living hits harder at a lower income",
        identityOverrides: { incomeLevel: "Under $25,000" },
        priorityOverrides: { costOfLiving: 90 },
  },
  {
        label: "If I earn more",
        description: "Models how high income changes city value",
        identityOverrides: { incomeLevel: "$250,000+" },
        priorityOverrides: { costOfLiving: 20 },
  },
  {
        label: "If I prioritise career",
        description: "Shifts weight heavily toward job market and tech hub access",
        priorityOverrides: { jobMarket: 90, costOfLiving: 30 },
  },
  {
        label: "If I prioritise climate",
        description: "Makes climate the dominant scoring factor",
        priorityOverrides: { climate: 90 },
  },
  {
        label: "If I go car-free",
        description: "Heavily weights public transit and walkability",
        priorityOverrides: { publicTransit: 90 },
  },
  ];
