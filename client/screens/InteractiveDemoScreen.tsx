...
// The demo screen is reachable from both the onboarding flow and the post-onboarding
// root stack (Profile → "Interactive Score Demo"). Accept either navigation prop type.
type Props =
    | {
...
export default function InteractiveDemoScreen({ navigation }: Props) {
  ...
  const cityScores = useMemo(() => {
        ...
        for (const cityId of DEMO_CITIES) {
                ...
                if (city) {
                          // Issue #3 fix: pass skipCache = true so that every slider adjustment
                  // produces an immediate recalculation without hitting the 5-minute TTL.
  ...
  const handleStartOnboarding = () => {
        (navigation as NativeStackNavigationProp<OnboardingStackParamList>).navigate(
                "IdentityStep1"
              );
  };
                                  ...
                                  {IDENTITY_PRESETS.map((preset) => (
                                      <Pressable
                                                      ...
                                                      <ThemedText
                                                                          ...
                                                                          style={{
                                                                                                color:
                                                                                                                        selectedPreset === preset.id ? "#FFFFFF" : theme.text,
                                                                                                fontWeight:
                                                                                                                        selectedPreset === preset.id ? "600" : "400",
                                                                          }}
                                                                        >
                                                        {preset.label}
                                                      </ThemedText>
