...
import { calculateCityScore, calculateAnonymousCityScore, applyPrivacyFilter } from "@/lib/scoring";
...
export default function CityDetailScreen({
  ...
  /**
     * Feature 1: Derive the filtered identity for testimonial matching.
     * Privacy settings are applied before passing identity to the testimonials
     * component so only dimensions the user has enabled are used for filtering.
     */
  const filteredIdentity = useMemo(() => {
        if (!profile || isAnonymousMode) return undefined;
        return applyPrivacyFilter(profile.identity, profile.privacySettings);
  ...
  if (isLoading) {
    ...
    const normalizedWeights = useMemo(() => {
          ...
          for (const key of Object.keys(profile.priorities)) {
                  weights[key] = (profile.priorities[key as keyof PriorityWeights] / total) * 100;
          }
          return weights;
    }, [profile]);
    ...
    const safetyNote = cityWithScore.personalizedScore.safetyPersonalisationNote;
    const hasHateCrimeData = !!cityWithScore.safety.hateCrimesByBiasCategory;
                        ...
                        <View
                          ...
                          {safetyNote ? (
                                                ...
                                                <View
                                                                style={[
                                                                                  styles.personalisationBanner,
                                                                  { backgroundColor: theme.warning + "10", borderColor: theme.warning + "25" },
                                                                                ]}
                                                              >
                                                              <View style={styles.personalisationBannerLeft}>
...
const styles = StyleSheet.create({
    ...
    // Feature 1: Safety personalisation banner
    ...
    personalisationBannerLeft: {
          marginTop: 2,
    },
    personalisationBannerText: {
          flex: 1,
          lineHeight: 18,
    },
    highlights: {
